...
public class MovieMgr {
	...
	List<com.my.movie.Movie> movies = new ArrayList<>();
    int index = 0 ;                      // 방의 번호.
	
	MovieMgr() {}						// 기본 생성자 
		
	//영화 정보 추가하기
	public void add(com.my.movie.Movie m) {
		movies.add(m);
//		movies[index++] = m;
	}
	...
	// 영화 제목으로 영화 찾기
	 void search(String title) {
		 ...
		 List<com.my.movie.Movie> findTitle = new ArrayList<>(); ;
		 for (int i =0 ; i < movies.size(); i++) {
	...
	void searchDirector(String name) {
		 ...
		 List<com.my.movie.Movie> findDirector = new ArrayList<>(); ;
		 for (int i =0 ; i < movies.size(); i++) {
	...
	List<com.my.movie.Movie> searchGenre(String genre) {
